...
$reportPath = "reports/kaggle_3m_multimodal_only_input_augmentation_ablation_seed42.md"
...
$experiments = @(
    ...
    [pscustomobject]@{
        ...
        Output = "runs/kaggle_3m_multimodal_only_e1_rgb_unet_no_augmentation_seed42"
    }
    [pscustomobject]@{
        Name = "E2-B"
        Config = "configs/kaggle_3m_multimodal_only_e2_flair_unet_augmentation.yaml"
        Output = "runs/kaggle_3m_multimodal_only_e2_flair_unet_augmentation_seed42"
    }
    [pscustomobject]@{
        Name = "M0-AB"
...
try {
    ...
    if ($Stage -in @("All", "Report")) {
        Write-Host "Generating clean input/augmentation ablation report..." -ForegroundColor Cyan
        Invoke-CheckedPython -Arguments @(
            "scripts/summarize_kaggle3m_clean_input_augmentation_ablation.py",
            "--output", $reportPath
        )
    }
}
...
if ($Stage -eq "All") {
    Write-Host "Completed E0/E1/E2/M0 training, testing, and report." -ForegroundColor Green
